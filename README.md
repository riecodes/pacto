...
`pacto unzip old-project` puts it all back and deletes the zip.
...
If a locked file interrupts the delete, re-running `pacto zip` finds the good
archive and finishes the job instead of re-zipping.

## Install

```bash
pip install "pacto[all]"                 # CLI, interactive picker, MCP server
pip install pacto                        # core only, no picker and no MCP
pipx install "pacto[all]"                # or uv tool install, to keep it off your global env
```
...
```bash
pacto                      # interactive picker, ranked by best candidate
pacto scan                 # list folders: size, idle days, git state
pacto scan --min-size 500MB --idle 60d
pacto zip old-project      # archive it, then delete the contents
pacto unzip old-project    # restore in place
```
...
- `--root D:\projects` overrides both, and `PACTO_ROOT` changes the default
...
- pacto never archives its own checkout.

## MCP server

```bash
claude mcp add pacto -- pacto-mcp
```

Or in `claude_desktop_config.json`:

```json
{ "mcpServers": { "pacto": { "command": "pacto-mcp" } } }
```
